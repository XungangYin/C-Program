// File Name: main.cpp
// Author: YXG
// Created Time: 2019年12月20日 星期五 15时47分31秒
/*
    并行测试
*/
...
namespace {
    ...
    int mandelbrot(const complex<float> &z0,const int max){
        complex<float> z = z0;
        for(int t = 0; t < max; ++t){
            if(z.real() * z.real() + z.imag()*z.imag() > 4.0) return t;
            z = z*z+z0;
         }
         return max;
    }
    ...
    int mandelbrotFormula(const complex<float> &z0,const int maxIter = 500){
     ...
     {
      public:
            ParallelMandelbrot(Mat &img,const float x1,const float y1,const
                float scaleX,const float scaleY):m_img(img),m_x1(x1),m_y1(y1),m_scaleX(scaleX),m_scaleY(scaleY){};
...
int main(int argc, char **argv){
   ...
   cout <<"Sequential MandelCount:"<<t2<<"s"<<endl;
   cout<<"Speed up:"<<t2/t1<<"X"<<endl;

    imwrite("Mandelbrot_parallel.png",mandelbrotImg);
    imwrite("Mandelbrot_sequency.png",mandelbrotImgSequential);
    return EXIT_SUCCESS;
}
